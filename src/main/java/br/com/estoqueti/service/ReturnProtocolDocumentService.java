...
public class ReturnProtocolDocumentService {
    ...
    private static final String ROOT_TEMPLATE_NAME = "PROTOCOLO DE DEVOLUCAO DE EQUIPAMENTO_NITROLUX_.docx";
    private static final String ROOT_TEMPLATE_NAME_WITH_ACCENT = "PROTOCOLO DE DEVOLU\u00C7\u00C3O DE EQUIPAMENTO_NITROLUX_.docx";
    private static final String RESOURCE_TEMPLATE_PATH = "/br/com/estoqueti/template/protocolo-devolucao-template.docx";
    private static final DateTimeFormatter MONTH_FORMATTER = DateTimeFormatter.ofPattern("MMMM", Locale.forLanguageTag("pt-BR"));

    public void generateDocument(ReturnProtocolDocumentData documentData, Path outputPath) {
        ...
        try {
            ...
            throw new IllegalStateException("Nao foi possivel gerar o arquivo do protocolo de devolucao.", exception);
        }
    }
    ...
    private byte[] buildDocumentXml(byte[] originalXml, ReturnProtocolDocumentData documentData) {
        try {
            ...
            fillDeclaration(document, documentData.employeeName(), documentData.employeeCpf());
            fillItemsTable(document, documentData);
            fillReasonSection(document, documentData.returnReason(), documentData.otherReason());
    ...
    private void fillProtocolNumber(Document document, String protocolNumber) {
        Element titleParagraph = findParagraphContaining(document, "PROTOCOLO DE DEVOLU");
    ...
    private void fillDeclaration(Document document, String employeeName, String employeeCpf) {
        ...
        text = replaceFirstBlank(text, employeeName);
        text = replaceFirstBlank(text, employeeCpf);
        setParagraphText(declarationParagraph, text);
    }

    private void fillItemsTable(Document document, ReturnProtocolDocumentData documentData) {
    ...
    private void fillReasonSection(Document document, ReturnProtocolReason returnReason, String otherReason) {
        fillReasonParagraph(document, "Desligamento da empresa", returnReason == ReturnProtocolReason.DESLIGAMENTO_EMPRESA, "Desligamento da empresa");
        fillReasonParagraph(document, "Altera", returnReason == ReturnProtocolReason.ALTERACAO_CARGO_FUNCAO, "Alteracao de cargo/funcao");
        fillReasonParagraph(document, "Substitui", returnReason == ReturnProtocolReason.SUBSTITUICAO_EQUIPAMENTO, "Substituicao do equipamento");

        String othersLabel = returnReason == ReturnProtocolReason.OUTROS
                ? "Outros: " + defaultValue(otherReason)
                : "Outros: __________________________";
        fillReasonParagraph(document, "Outros:", returnReason == ReturnProtocolReason.OUTROS, othersLabel);
    }

    private void fillReasonParagraph(Document document, String searchText, boolean selected, String label) {
        Element paragraph = findParagraphContaining(document, searchText);
        if (paragraph == null) {
            return;
        }
        setParagraphText(paragraph, (selected ? "(X) " : "( ) ") + label);
    }

    private void fillDateLine(Document document, LocalDate returnedDate) {
        Element dateParagraph = findParagraphContaining(document, "Recife,");
        if (dateParagraph == null) {
            return;
        }

        String monthName = MONTH_FORMATTER.format(returnedDate);
        String line = "Recife, " + returnedDate.getDayOfMonth() + " de " + monthName + " de " + returnedDate.getYear() + ".";
        setParagraphText(dateParagraph, line);
    }

    private void fillSignatureTable(Document document, String receiverName, String receiverRole, String receiverCpf) {
        Element signatureTable = findTableContaining(document, "Assinatura do Respons");
        ...
        setFirstCellText(rows.get(2), "Nome: " + receiverName);
        setFirstCellText(rows.get(3), "Cargo: " + receiverRole);
        setFirstCellText(rows.get(4), "CPF: " + receiverCpf);
    }

    private void setFirstCellText(Element row, String value) {
        List<Element> cells = childElements(row, "tc");
        if (cells.isEmpty()) {
            return;
        }
        setCellText(cells.get(0), value);
    }
